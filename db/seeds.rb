...
require 'csv'
  ...
  CSV.foreach ('seed_csvs/markets.csv') do |csv_obj|
    Market.create(id: csv_obj[0].to_i, name: csv_obj[1], address: csv_obj[2], city: csv_obj[3], county: csv_obj[4], state: csv_obj[5], zip: csv_obj[6].to_i)
  end

  CSV.foreach ('seed_csvs/vendors.csv') do |csv_obj|
    Vendor.create(id: csv_obj[0].to_i, name: csv_obj[1], no_employees: csv_obj[2].to_i, market_id: csv_obj[3].to_i)
  end
